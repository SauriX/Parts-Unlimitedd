...
const Proceding = {
  ...
  getTaxData: (recordId: string): Promise<ITaxData[]> => requests.get(`MedicalRecord/taxData/${recordId}`),
  ...
  createTaxData: (taxData: ITaxData): Promise<string> => requests.post("/MedicalRecord/taxData", taxData),
  update: (pack: IProceedingForm): Promise<IProceedingList> => requests.put("/MedicalRecord", pack),
  updateTaxData: (taxData: ITaxData): Promise<void> => requests.put("/MedicalRecord/taxData", taxData),
