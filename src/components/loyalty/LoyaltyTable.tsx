    ...
    <Space direction="vertical">
      <DatePicker.RangePicker
        onChange={(e: any) => {
          if (e) {
            setNewDate(
              `${moment(e[0]).format("YYYY/MM/DD")} - ${moment(e[1]).format(
                "YYYY/MM/DD"
              )}`
            );
        ...
        onClick={() => {
          const loyalty: ILoyaltyForm = {
            ...
            fechaInicial: new Date(
              moment(newDate.split(" - ")[0]).toISOString().split("T")[0]
            ),
            fechaFinal: new Date(
...

    {
      ...
      render: (value) => (
        <IconButton
          ...
          onClick={() => {
            navigate(
              `/loyalties/${value}?${searchParams}&mode=edit&search=${
                searchParams.get("search") ?? "all"
              }`
            );
          }}
